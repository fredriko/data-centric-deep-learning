...
class TrainIdentifyReview(FlowSpec):
  ...
  def crossval(self):
    ...
    for train_index, test_index in kf.split(X):
      ...
      X_train = torch.from_numpy(X_train).float() # pylint: disable=E1101
      X_test = torch.from_numpy(X_test).float() # pylint: disable=E1101
      y_train = torch.from_numpy(y_train).float() # pylint: disable=E1101
      y_test = torch.from_numpy(y_test).float() # pylint: disable=E1101
      ...
      probs_ = torch.cat(probs_).squeeze(1).numpy() # pylint: disable=E1101
