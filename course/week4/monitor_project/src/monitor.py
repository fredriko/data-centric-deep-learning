...
def get_ks_score(tr_probs, te_probs):
  ...
  # ============================
  _, score = ks_2samp(te_probs.cpu().numpy(), tr_probs.cpu().numpy())
  return score
...
class MonitoringSystem:
  ...
  def calibrate(self, tr_probs, tr_labels, te_probs):
    ...
    tr_probs_cal = calibrated_model.fit_transform(tr_probs.cpu().numpy(), tr_labels.cpu().numpy())
    te_probs_cal = calibrated_model.transform(te_probs.cpu().numpy())
